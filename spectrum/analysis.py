...
def WriteTable(object, satelite, simultaneous, gaussian, apec):
    ...
    with open("parameter.txt", mode="w") as fout:
        ...
        fout.write("{0:<80}".format("$"+"{0:0<4}".format(round(1e-02*xspec.AllModels(1)(4) .values[0],2))+"_{-"+"{0:0<4}".format(round(1e-02*(1.645*statistics.stdev(hydrogen))                                ,2))+"}^{+"+"{0:0<4}".format(round(1e-02*(1.645*statistics.stdev(hydrogen))                                ,2))+"}$")+"& "    )
